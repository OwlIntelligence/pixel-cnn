...
FLAGS.z_dim = 8*8*10
FLAGS.save_dir = "/data/ziz/jxu/models/svae-64"
...
def inference_network(x):
    with tf.variable_scope("inference_network"):
        net = tf.reshape(x, [-1, 64, 64, 3]) # 64x64x3
...
if FLAGS.debug:
    train_data = celeba_data.DataLoader(FLAGS.data_dir, 'valid', FLAGS.batch_size*FLAGS.nr_gpu, shuffle=True, size=64)
else:
    train_data = celeba_data.DataLoader(FLAGS.data_dir, 'train', FLAGS.batch_size*FLAGS.nr_gpu, shuffle=True, size=64)
test_data = celeba_data.DataLoader(FLAGS.data_dir, 'valid', FLAGS.batch_size*FLAGS.nr_gpu, shuffle=False, size=64)
