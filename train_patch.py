...
def sample_from_model(sess, data=None, **params):
    ...
    if args.deconv_z:
        ...
        feed_dict.update({indices[i]:np.array([[0,0,0,0], [args.batch_size, 5,5,64]], dtype=int32) for i in range(args.nr_gpu)})
...
def make_feed_dict(data, init=False, **params):
    ...
    if init:
        ...
        if args.deconv_z:
            ...
            feed_dict.update({indices_init:np.array([[0,0,0,0], [args.init_batch_size, 5,5,64]], dtype=np.int32)})
        if args.global_conditional:
        ...
        if args.deconv_z:
            ...
            feed_dict.update({zhs[i]: z[i].reshape((z[i].shape[0],8,8,10)) for i in range(args.nr_gpu)})
            feed_dict.update({indices[i]:np.array([[0,0,0,0], [args.batch_size, 5,5,64]], dtype=np.int32) for i in range(args.nr_gpu)})
        if args.global_conditional:
            global_lv = np.split(global_lv, args.nr_gpu)
            feed_dict.update({ghs[i]: global_lv[i] for i in range(args.nr_gpu)})
        if args.spatial_conditional:
